...
- 크롬(헤더·카드·버튼)은 조용하게, **색·모션 예산은 데이터 시각화(파급 그래프·지식그래프)에 몰아준다.**
- 카드 기반 2단 레이아웃(콘텐츠 좌 / 요약·현황 우). 좌상단 로고 `수혜주.com`, 우상단 상태 표시.
- 분석 페이지 섹션 순서(F-16 #86 개편): AI 요약 → **영향력 확산 그래프**(핵심 산출물 — 구 히트맵의 영향도 역할 통합) → 전망 분석 → 산업 지식그래프. 각 시각화 섹션 상단에 "이 그림이 말하는 것" 한 문단 해설을 둔다.
...
- 등락·신호 카드·그래프 영향도 전부 이 규칙. **범례 필수 병기**, 색 외에 화살표·부호(+/−) 병용.
...
- **확산 그래프**: 원점→1차→2차→3차 열 배치, 단계색 점 + **영향도 텍스트 라벨**(↗/↘ + '강한/보통/약한 영향을 받아요', 색=방향 positive/negative, 채도=강도 — 강할수록 원색, 약할수록 잉크 혼합(color-mix)로 밝게 탈색해 가독성 유지. 등락%는 F-16에서 제거), 연결선 hover 근거 툴팁, Top5 툴팁 종목 클릭 시 주가 추이 모달. 캔버스 렌더 시 노드 라벨 800/600 weight.
- **주가 추이 모달**: 일봉 라인(색=기간 등락 방향) + **뉴스 발행일 고정 기준선**(blue-bright 점선+점+라벨, 발행일 미확인 시 '분석일') + hover 크로스헤어(muted-foreground 실선+점)로 날짜·시/고/저/종 상세 표시. 가격 라벨은 muted-foreground + 단위(원/$) 병기, 핵심 수치는 "발행일 종가 대비 이후 최고/최저 %"(고가/저가 기준)로 부호·화살표 병기 — 구간 첫날 대비("기간 대비")는 기준점이 자의적이라 쓰지 않는다.
- **칩(pill)**: 섹터·키워드 공통, `{components.chip}`.
- **배경 파티클(장식)**: 메인 페이지 히어로 뒤 canvas 네트워크. 노드 색은 tier1~3·positive·negative 토큰을 런타임에 읽어 **흰색 55% 블렌드**(파스텔화)해서 그린다 — 토큰 원본 hex는 건드리지 않고 캔버스 렌더링에서만 순화. 연결선은 `--tier1`, 별빛(흐르는 점)은 `--ink`를 블렌드 없이 그대로 사용. 순수 장식(`aria-hidden` + `pointer-events-none`)이라 positive/negative를 의미 없이 색상 팔레트로만 섞어 써도 되지만, 데이터 시각화(신호 카드·그래프 영향도 등)에는 이 예외를 적용하지 않는다.
