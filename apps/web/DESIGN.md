...
- **배경 파티클(장식)**: 메인 페이지 히어로 뒤 canvas 네트워크. 노드 색은 tier1~3·positive·negative 토큰을 런타임에 읽어 **흰색 55% 블렌드**(파스텔화)해서 그린다 — 토큰 원본 hex는 건드리지 않고 캔버스 렌더링에서만 순화. 연결선은 `--tier1`, 별빛(흐르는 점)은 `--ink`를 블렌드 없이 그대로 사용. 순수 장식(`aria-hidden` + `pointer-events-none`)이라 positive/negative를 의미 없이 색상 팔레트로만 섞어 써도 되지만, 데이터 시각화(신호 카드·히트맵 등)에는 이 예외를 적용하지 않는다.
