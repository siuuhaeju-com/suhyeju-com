...
| `feat/#이슈번호-기능명`     | **기능 작업 브랜치** — 이슈 단위로 생성           |
| `refactor/#이슈번호-작업명` | **리팩토링 작업 브랜치** — 동작 유지·구조 개선용 |

**흐름:** `feat|refactor/#이슈번호-작업명` → `dev` → `prd`

- 작업은 반드시 **이슈 번호가 포함된 작업 브랜치**에서 한다. 기능은 `feat/#이슈번호-기능명`, 리팩토링은 `refactor/#이슈번호-작업명`을 쓴다. (예: `feat/#12-login-form`, `refactor/#101-analysis-architecture`)
